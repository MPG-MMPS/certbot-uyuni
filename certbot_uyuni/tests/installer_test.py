...
def _mock_proc(returncode: int = 0, stdout: bytes = b"",
               stderr: bytes = b"") -> mock.MagicMock:
    proc = mock.MagicMock()
...
class UyuniInstallerTest(unittest.TestCase):
    ...
    @mock.patch.object(UyuniInstaller,
                       "_container_running", return_value=True)
    @mock.patch.object(UyuniInstaller,
                       "_cmd_exists", return_value=True)
    def test_prepare(self, _mock_cmd: Any,
                     _mock_running: Any) -> None:
        self.installer.prepare()

    @mock.patch.object(UyuniInstaller,
                       "_cmd_exists", return_value=False)
    def test_prepare_missing_binary(self,
                                    _mock_cmd: Any) -> None:
        self.assertRaises(
    ...
    def test_deploy_cert_error(self, mock_run: Any) -> None:
        ...
        self.assertRaises(
            errors.PluginError, self.installer.deploy_cert,
            "example.com", "/cert.pem", "/key.pem",
            "/chain.pem", "/fullchain.pem")
    ...
    def test_restart_failure(self, mock_run: Any) -> None:
        mock_run.return_value = _mock_proc(returncode=1, stderr=b"fail")
        self.assertRaises(
            errors.MisconfigurationError, self.installer.restart)

    @mock.patch("time.sleep")
    @mock.patch("subprocess.run")
    def test_restart_waits_for_healthy(
            self, mock_run: Any,
            _mock_sleep: Any) -> None:
        self.installer.conf = mock.MagicMock(  # type: ignore[method-assign]
            return_value=10)
    ...
    def test_enhance_raises(self) -> None:
        self.assertRaises(
            errors.PluginError, self.installer.enhance,
